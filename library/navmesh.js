...
(function($) {
	...
	var helpVector = new navmesh.Vector2f(0, 0);

	function Triangulation(path, group) {
		this.triangleV = null;
		this.group = group;
		this._build(path);
	}
	...
	Triangulation.prototype = {
				...
				for(i=0, len=path.length; i<len; i++) {
					if(path[i] === Triangulation.COMBINE_ITEM) {
						if(poly.length > 0) {
							polyArr.push(new navmesh.Polygon(poly.length, poly));
							delaunay = new navmesh.Delaunay();
							this.triangleV = this.triangleV.concat(delaunay.createDelaunay(polyArr));
							polyArr = [];
							poly = [];
						}
					} else {
		...
		isCoordsIn : function(x, y) {
			helpVector.x = x;
			helpVector.y = y;
			return this.isPointIn(helpVector);
		},
...
(function($) {
	...
	function Path(collisionPaths, isTriangleData, cellV) {
		this.navMesh = null;
        this.nearByCellCache = {};
        if(cellV) {
            this.cellV = cellV;
        } else {
            if (isTriangleData) {
                this._buildCellVByRawData(collisionPaths);
            } else {
	...
	Path.prototype = {
		isPointIn : function(x, y) {
            return this.navMesh.getCellByPoint(new $.Vector2f(x, y));
		},
        getNearByCell : function(pt) {
            var cacheKey = 'cache_' + pt.x + '_' + pt.y;
            var cache = this.nearByCellCache[cacheKey];
            if(cache) {
                return cache;
            }
            var cellV = this.cellV;
            var i, len;
            ...
            for(i= 0, len=cellV.length; i<len; i++) {
                cell = cellV[i];
                if(cell.center) {
                    distance = cell.center.distanceSquared(pt);
            ...
            if(nearByCell) {
                this.nearByCellCache[cacheKey] = nearByCell;
            }
            return nearByCell;
        },
		find : function(from, to, findWalkable) {
            var iteraterCount = findWalkable ? 100 : 1;
			var path = null;
			var walkable = true;
			var temp = { x: 0, y: 0 };
			var end = { x : to.x, y : to.y };
            var distance = Math.sqrt(Math.pow(to.x-from.x, 2) + Math.pow(to.y-from.x, 2));
            if(distance <= 20) {
                return { walkable: true, path: null };
            }
            var nearByCell = this.getNearByCell(to);
            var nearByCellCenter = nearByCell.center;
            var dir = { x : end.x-nearByCellCenter.x, y : end.y-nearByCellCenter.y };
            var len;
            vectorNomalize(dir);
			while(!path) {
				iteraterCount --;
				if(iteraterCount < 0) return { walkable: false, path: null };
		...
		_buildTriangleV : function(collisionPaths) {
			this.triangleV = new $.Triangulation(collisionPaths).triangleV;
		},
        ...
        _buildCellVByRawData : function(triangleData) {
            var cellV = [];
            var trg;
            var cell;
            var i, j, len, len2;
            for (j=0, len=triangleData.length; j<len; j++) {
                trg = triangleData[j];
                cell = new navmesh.Cell(
    ...
    function timeChuck(arr, callback, onFinished) {
        var i=0;
        var start = Date.now();
        var use = 0;
        var len = arr.length;
    ...
    $.Path.asyncBuild = function(triangleData, callback) {

        var cellV = [];

        timeChuck(triangleData, function(trg, index) {
            var cell = new navmesh.Cell(
                new navmesh.Vector2f(trg.a.x, trg.a.y),
                new navmesh.Vector2f(trg.b.x, trg.b.y),
                new navmesh.Vector2f(trg.c.x, trg.c.y)
            );
            ...
            timeChuck(cellV, function(cell) {
                ...
                callback && callback(new Path(null, null, cellV));
            });
        });
